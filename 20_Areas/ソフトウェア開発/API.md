...
要求応答型でHTTPを利用でき、いずれの方式も選べる状況では、クライアント、データの形、通信性能、運用主体を基準に選択する。

### REST

- URLでリソース、HTTPメソッドで操作を表し、通常はJSONを用いる。
- HTTPのキャッシュ、条件付き要求、ステータスコードをそのまま活用でき、内容を読みやすく調査しやすい。
- ツールが豊富で学習負担が小さく、公開APIの既定候補になる。
- 複数回の要求が必要になりやすく、過剰取得や取得不足が起こる場合がある。

### gRPC

- Protocol BuffersとHTTP/2を用いる手続き呼び出し方式である。
- 通信量が小さく低遅延で、定義ファイルから多言語のコードを生成できるため型の一貫性を保ちやすい。
- 両端を管理できる社内サービス間通信に向く。
- ブラウザから直接呼ぶにはgRPC-WebやConnect-RPCなどが必要で、バイナリ形式のため公開APIの調査には向きにくい。

### GraphQL

- 単一エンドポイントに対し、クライアントが必要なデータの形を指定する。
- 過剰取得や追加要求を減らせ、スキーマを契約として扱える。項目追加も既存クエリを壊しにくい。
- URL単位のキャッシュ、認可、レート制限が難しく、内部クエリの分散によって性能が低下する場合がある。
- 単純なリソース操作には過剰な設計になりうる。
...
|単純さ、互換性、公開API|REST|
|社内通信の性能、型の一貫性|gRPC|
|画面ごとに異なる柔軟な取得|GraphQL|

- 一つに統一する必要はなく、公開APIはREST、社内通信はgRPC、フロントエンド向け集約はGraphQLのように併用できる。
- 唯一の正解はなく、クライアントと解決したい問題を基準に選ぶ。
