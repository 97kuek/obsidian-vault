...
# per docs/standards/vault-structure.md: do not touch .obsidian/ except the CSS snippet.
...
case "$fp" in
  */.obsidian/snippets/vault-custom.css)
    exit 0 ;;
  */.obsidian/*|.obsidian/*)
    printf '%s\n' "Blocked: .obsidian/ は Obsidian 設定のため保護されています（docs/standards/vault-structure.md）。編集してよいのは .obsidian/snippets/vault-custom.css のみです。" >&2
    exit 2 ;;
  *)
    exit 0 ;;
esac
