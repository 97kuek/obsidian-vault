...
| `/proofread` または「このファイルを添削して」 | 随時 | 指定ファイル・フォルダの文章を、だ・である調、箇条書き、表、コードブロック中心に整形 |
...
- Obsidian標準テーマを土台に、暖色の背景・文字・アクセントだけを調整する。
- 外部フォント、アニメーション、特定プラグイン専用CSS、ファイルやフォルダの強制非表示は使わない。
- 見出し、引用、コード、コールアウト、表、タグは最小限の可読性調整だけを行う。
- ライト・ダーク・モバイルの各表示へ対応する。
